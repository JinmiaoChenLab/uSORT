...
#' parameter for \code{STS_sorting_wrapper} method
#' @param alpha window size which is given in  percentage of total no. of cells,
#' parameter for \code{c2c_dist_uneveness}
#' @param local_sort a logical stating if local sorting feature is implemented: (1) turn on, 
#' (0) turn off
...
sorting_wrapper<-function(expr, no_randomization = 10, local_sort = FALSE, 
                          window_perc_range = c(0.1,0.9), alpha = 0.2,
                          window_size_incre_perct = 0.05, 
                          sorting_method = NULL, baseNm = 'uSPIN', sigma_width = 1,
                          data_type = NULL){
    ...
    if(sorting_method == 'STS') global_res<-STS_sorting_wrapper(expr, no_randomization= no_randomization)
    else if(sorting_method == 'neighborhood') global_res<-neighborhood_sorting_wrapper(expr, no_randomization= no_randomization, sigma_width = sigma_width)
    ...
    if(local_sort==TRUE){
        ...
        return(data.frame('SampleID'=rownames(global_ordering), 'GroupID' = rep('untitled',n)))
    }#end local sorting
    
    else {return(data.frame('SampleID'=rownames(global_ordering), 'GroupID' = rep('untitled',n)))}
}
