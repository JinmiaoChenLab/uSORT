...
sWanderlust <- function(exp_data,
    ...
    autoSPIN_order <- autoSPIN(t(exp_data$log2ex_data),
    ...
    ## try both ends as starting point with wanderlust and find the best
    ## wanderlust preprocess
    data <- t(exp_data$log2ex_data)
    medColSums <- median(rowSums(data))
    data <- t(apply(data, 1, function(x) x/sum(x)*medColSums))
    nonzero_genes <- rowSums(data) > 0
    ...
    wishbone1 <- Rwanderlust(data = dm[ ,2:(diffusionmap_components + 1)],
                           s = startingCell1,
                           l=l, k=k,
                           num_waypoints = num_waypoints,
                           waypoints_seed = waypoints_seed)

    wanderlust_ordering1 <- data.frame('SampleID' = wishbone1$Order,
                                       'GroupID' = 'untitled')
    wanderlust_exp1 <- updateSampleListFromList(exp_data, wanderlust_ordering1)
    ...
    wishbone2 <- Rwanderlust(data = dm[ ,2:(diffusionmap_components + 1)],
                           s = startingCell2,
                           l=l, k=k,
                           num_waypoints = num_waypoints,
                           waypoints_seed = waypoints_seed)

    wanderlust_ordering2 <- data.frame('SampleID' = wishbone2$Order,
                                       'GroupID' = 'untitled')
    wanderlust_exp2 <- updateSampleListFromList(exp_data, wanderlust_ordering2)
    c2 <- STS_sortingcost(expr = t(wanderlust.exp2$log2ex_data))

    # Pick an optimal ordering
    if (c1 < c2) {
        sWanderlust_order <- wanderlust_ordering1
        cat('Starting cell ', startingCell1, ', cost=', c1 , '(driver genes)\n')
    } else {
        sWanderlust_order <- wanderlust_ordering2
        cat('Starting cell ', startingCell2, ', cost=', c2 , '(driver genes)\n')
    }
...
Rwanderlust <- function(data, s,
    ...
    if(verbose)
        cat(paste0("    lNN computed in: ", round(lnn_time[3], 2), " seconds\n"))
    ...
    for(graph_iter in seq_len(num_graphs)){
        ...
        converged <- FALSE; user_break <- FALSE
        realign_iter <- 1
        while(converged == FALSE && user_break == FALSE){
            ...
            fpoint_corr <- cor(t[[realign_iter+1]], t[[realign_iter]], method = "pearson")
            if (verbose)
                cat(paste0('      Correlation with previous iteration: ', round(fpoint_corr, 6), "\n"))
            converged <- fpoint_corr > 0.9999

            if (realign_iter > 20){
                # break after too many alignments - something is wrong
                user_break <- TRUE
                warning(paste('Force exit after', realign_iter, 'iterations.\n'))
            }

        }

        iter_traj <- t[[realign_iter+1]]  ## +1 here
        iter_traj <- (iter_traj - min(iter_traj)) / (max(iter_traj) - min(iter_traj))
    ...
    return(list(Trajectory = trajectory, Order = order, Waypoints = waypoints))
}
...
spdists_klnn <- function(lnn, k){
    ...
    klnn$nn.dists <- matrix(lnn$nn.dists[sampleID], byrow = TRUE, ncol = k)
    return(klnn)
}
...
trajectory_landmarks <- function(knn, data, s,
                                 partial_order=NULL,
                                 waypoints=250,
                                 waypoints_seed = 123,
                                 metric='euclidean',
    ...
    if(length(waypoints) == 1 && is.numeric(waypoints)){
        ...
        if(band_sample){
            ...
            while(prc > 0.08){
                band <- (dijkstra >= ((prc-window_size)*max_dist)) & (dijkstra <= prc*max_dist)
                n_opts <- c(n_opts, sample(which[band],
                                           min(sum(band), waypoints-length(partial_order)),
                                           replace = FALSE))
                prc <- prc - window_size
            }
        }
        ...
        waypoints <- sample(n_opts, waypoints-length(partial_order), replace=FALSE)
    }
    ...
    if(flock_waypoints){
        ...
        for(f in seq_len(flock_waypoints)){
            n_neighbors <- 20
            idx <- nn2(data, data[waypoints, ,drop=FALSE],
                       n_neighbors, searchtype = "standard")$nn.idx
            for(i in seq_along(waypoints)){
                med_data = matrix(apply(data[idx[i,], ], 2, median),nrow=1)
                waypoints[i] <- nn2(data, med_data, 1)$nn.idx[1,1]
            }
        }
    }
    ...
    ## add extra landmarks if user specified
    #l <- unique(c(partial_order, waypoints)) ## start with starting point
    l <- c(partial_order, waypoints)
    #print(table(l))

    ## calculate all shortest paths
    g <- graph.adjacency(knn, mode="undirected", weighted = TRUE)
    dist <- list()
    for(i in seq_len(length(l))){
        sp_dist <- shortest.paths(g, v=l[i], weights=E(g)$weight)[1,]
        unreachable <- is.infinite(sp_dist)
        sp_dist[unreachable] <- max(c(sp_dist[!unreachable], laply(dist, max)))
        dist[[i]] <- sp_dist
    }

    ## adjust paths according to partial order by redirecting
    ## leave out here with partial_order=NULL
    traj <- do.call(rbind, dist)
    if (length(l) > length(partial_order)){
        traj <- realign_trajectory(dist, dist, l, traj,
                                   length(partial_order)+1,length(l), 1)
    }
...
realign_trajectory <- function(t, dist, landmarks, traj,
...
weighting_scheme <- function(dist,
                             voting_scheme = c("uniform", "exponential", "linear")){
    voting_scheme <- match.arg(voting_scheme)
    W_full <- do.call(rbind, dist)
    switch(voting_scheme,
           uniform = {
               W_full[] <- 1
           },
           exponential = {
               std <- apply(W_full, 2, function(x){sqrt(sum((x - mean(x))^2)/length(x))})
               sdv <- mean(std) * 3
    ...
    W_full <- apply(W_full, 2, function(x){x/sum(x)})
    return(W_full)
}


#' @importFrom RSpectra eigs_sym
diffusionmap <- function(data,
                         data_type = c("scSeq", "massCyt"),
                         knn=10,
                         epsilon=1,
    ...
    switch(data_type,
           scSeq = {
               ...
               ## center and scale looks weird here
               ## but just follow the python implementation
               data <- data - min(data)
               data <- data/max(data)
    ...
    DV <- eigs_sym(TT, k=n_diffusion_components,
                   opt = list(tol=1e-4, maxiter=1000,retvec=TRUE))
...
pca <- function(x, n_components = 100){
    ...
    if(ncol(x) < nrow(x)){
        ...
        # if #gene > #cell, we better use this matrix for the eigen decomposition
        c <- x %*% t(x) / nrow(x)
    }
    r <- eigen(c)
    l <- r$values; m <- -r$vectors
    ...
    if (ncol(x) >= nrow(x)){
        m <- t(x) %*% m / matrix(sqrt(nrow(x)*l), byrow = TRUE, nrow = nrow(t(x)), ncol = ncol(m))
    }
