...
uSORT_write_results <- function(uSORT_results, project_name, result_directory){
    ...
    pre_data <- uSORT_results$trimmed_log2exp[uSORT_results$preliminary_sorting_order, uSORT_results$preliminary_sorting_genes]
    priliminary_c2c_dist <- distance.function(pre_data)
    pdf(paste0(project_name, "_distance_heatmap_preliminary.pdf"))
    heatmap.2(priliminary_c2c_dist,
              dendrogram = 'none',
              trace = 'none',
              col = colorpanel(10, "red", "yellow", "blue"),
              Rowv = F,
              Colv = F,
              labCol = NA ,
              labRow = NA ,
    ...
    pdf(paste0(project_name, "_final_driver_genes_profiles.pdf"), width = 8)
