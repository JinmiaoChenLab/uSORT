...
pca_gene_selection <- function(data){
    ...
    ## select genes
    rotation_matrix <- as.data.frame(abs(pca$rotation[, select_PCs, drop = FALSE]))
...
driving_force_gene_selection <- function(cds, scattering.cutoff.prob = 0.75,
                              driving.force.cutoff = NULL, qval_cutoff = 0.05,
                              min_expr = 0.1, data_type = c("linear", "cyclical"), nCores = 1){
    ...
    no.genes <- nrow(trimmed_cds)
    cat(paste0('  No. of driver genes @qval(', qval_cutoff, ') = ', no.genes,'\n'))

    ## =====Filter driver genes for large driving force==========
    change <- ifelse(data_type == 'cyclical', 'cyclic.driving.force', 'linear.driving.force')
    trimmed_diff_test_res <- diff_test_res[rownames(diff_test_res) %in% rownames(trimmed_cds@assayData$exprs),]
    driving_force <- data.frame('GeneID'=rownames(trimmed_diff_test_res), 'Score'=trimmed_diff_test_res[,change])
    driving_force$Score[is.infinite(driving_force$Score)] <- 0
    driving_force <- driving_force[order(driving_force$Score, decreasing=T), ,drop=F]
    ...
    if(!is.null(driving.force.cutoff)){
        ...
        selected_driver_genes <- driving_force[driving_force$Score > driving.force.cutoff, 'GeneID']
...
diff_test_helper1 <- function (x, fullModelFormulaStr, reducedModelFormulaStr,
                               expressionFamily, lowerDetectionLimit=0.1,
                               type_ordering = 'linear'){
