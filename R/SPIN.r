...
SPIN <- function(data, sorting_method = c("STS", "neighborhood"), sigma_width = 1){
...
#' properOrdering_cost
STS_sortingcost<-function(expr = NULL, method= c('Euclidean','Correlation','eJaccard','none')){
    ## Define parameters & constant
    n<-nrow(expr)
    I<-diag(n)
...
#' properOrdering_cost
neighborhood_sortingcost<-function(expr = NULL, sigma_width=1, method= c('Euclidean','Correlation','eJaccard','none')){
    ## Define parameters & constant
    mat_size <- nrow(expr)
    s <- seq(mat_size)
    m <- rep(s,mat_size)
