...
sWanderlust <- function(data, data_type = c("linear", "cyclical"), 
    ...
    autoSPIN_order <- autoSPIN(data, data_type = data_type, sorting_method = SPIN_option, 
        sigma_width = sigma_width, alpha = alpha)
...
wanderlust_wrapper <- function(data, s, diffusionmap_components = 4, 
    l = 15, k = 15, num_graphs = 1, num_waypoints = 150, waypoints_seed = 123, 
    flock_waypoints = 2) {
    ...
    res <- Rwanderlust(data = dm[, 2:(diffusionmap_components + 
        1)], s = s, l = l, k = k, num_graphs = num_graphs, num_waypoints = num_waypoints, 
        waypoints_seed = waypoints_seed, flock_waypoints = flock_waypoints)
...
Rwanderlust <- function(data, s, l = 15, k = 15, num_graphs = 1, 
    ...
    metric = "euclidean", voting_scheme = "exponential", band_sample = FALSE, 
    ...
    for (graph_iter in seq_len(num_graphs)) {
        ...
        klnn <- sparseMatrix(i = sm_i, j = sm_j, x = as.vector(t(klnn$nn.dists)))
        ## Make the graph undirected
        klnn[cbind(sm_j, sm_i)] <- klnn[cbind(sm_i, sm_j)]
        
        ## Detect landmarks and trajectory
        traj_l <- trajectory_landmarks(klnn, data, s, partial_order = partial_order, 
...
diffusionmap <- function(data, data_type = c("scSeq", "massCyt"), 
    knn = 10, epsilon = 1, n_diffusion_components = 10, n_pca_components = 15, 
    markers = NULL) {
