...
#' @param waypoints_seed The seed for reproducing the results.
#' @param flock_waypoints The number of times for flocking the waypoints, default is 2.
wanderlust_wrapper <- function(data, s,
                               diffusionmap_components = 4,
                               l=15, k=15,
                               num_graphs=1,
                               num_waypoints=150,
...
trajectory_landmarks <- function(knn, data, s,
    ...
    g <- graph_from_adjacency_matrix(knn, mode="undirected", weighted = TRUE)
    dist <- list()
...
diffusionmap <- function(data,
    ...
    DV <- eigs_sym(TT, k=n_diffusion_components,
                   opts = list(tol=1e-4, maxiter=1000,retvec=TRUE))
